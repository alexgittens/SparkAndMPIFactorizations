...
lazy val BinToNumpy = taskKey[Unit]("Convert binary dump of PCs to numpy dataset")
BinToNumpy <<= (assembly in Compile) map {
  (jarFile: File) => s"src/runbintonumpy.sh ${jarFile} "!
}

lazy val BinToNumpy2 = taskKey[Unit]("Convert binary dump of PCs to numpy dataset")
BinToNumpy2<<= (assembly in Compile) map {
  (jarFile: File) => s"src/runbintonumpy2.sh ${jarFile} "!
}

lazy val submit = taskKey[Unit]("Compute truncated PCA")
